if (window.ethereum) {
  ...
  var contractAddress = '0x6b2d802b3EA9D56E02870786f371F54f82D89581';
  ...
  web3.eth.getCoinbase(function(err, account){
    ...
    contract.methods.getInfo2(2).call().then(function(view){
        ...
        contract.methods.getInfo(3).call().then(function(view){
          ...
          var template = "<tr><th>" + id + "</th><td>" + name + "</td><td>" + ppeCount + "</td></tr>"
          document.getElementById('distributionResult1').innerHTML = template;
          
          var centerOption = "<option value ='" + id + "' >"+ name + "</option>"
          document.getElementById('centerSelects').innerHTML = centerOption;
        });
        
        contract.methods.getInfo(4).call().then(function(view){
          ...
          var template = "<tr><th>" + id + "</th><td>" + name + "</td><td>" + ppeCount + "</td></tr>"
          document.getElementById('distributionResult2').innerHTML = template;
          
          var centerOption = "<option value ='" + id + "' >"+ name + "</option>"
          document.getElementById('centerSelect').innerHTML = centerOption;
        });
        ...
        function sendTransaction(){
          ...
          contract.methods.sendToAuthor(centerId,ppeAmount).send( {from: account} ).then(function(tx){
             contract.methods.getInfo(3).call().then(function(view){
                 var id = view[0];
                 var name = view[1];
                 var ppeCount = view[2];
      ...
      function sendTransactions(){
        ...
        var ppeAmount = $('#amountSelect').val();
    ...
    // //withdraw
    // function withdrawPpe(){
    //   var id = $('#yourId').val();
    //   var key = $('#privateKey').val();
    //   var amount = $('#amount').val();

    //   contract.methods.withdraw(id,key,amount).send({from:account}).then(function(res){
    //     location.reload();
    //   });
    // }